...
from leetcode_coach_v2.domain.services import CoachDomain as SyncCoachDomain
    ...
    async def search_problem_pool(
        ...
        def query(session: Session):
            statement = select(V2Problem).where(
                V2Problem.eligible == True, V2Problem.solved == False  # noqa: E712
            )
            difficulties = [
                str(value).lower()
            ...
            for tag in filters.get("exclude_tags", []):
                ...
                if normalized:
                    statement = statement.where(
                        ~func.lower(V2Problem.tags).contains(normalized)
                    )
...
class PostgresAgentSession:
    ...
    async def get_items(self, limit: int | None = None) -> list[dict[str, Any]]:
        def execute():
            with Session(self.engine) as session:
                statement = select(V2ConversationItem).where(
