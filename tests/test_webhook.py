...
from leetcode_coach import main
...
async def test_duplicate_webhook_distinguishes_inflight_from_handled(
    ...
    response = await main.telegram_webhook(_Request(), x_telegram_bot_api_secret_token="secret")
...
async def test_invalid_webhook_secret_is_ignored_without_touching_state(monkeypatch) -> None:
    ...
    response = await main.telegram_webhook(_Request(), x_telegram_bot_api_secret_token="wrong")
