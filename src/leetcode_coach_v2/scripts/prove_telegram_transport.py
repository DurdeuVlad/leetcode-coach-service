...
from leetcode_coach_v2.integrations import telegram
from leetcode_coach_v2.rendering import proposal_keyboard, render_proposal_html
...
def staging_settings_from_env() -> StagingTelegramSettings:
    token = os.environ.get("V2_STAGING_TELEGRAM_BOT_TOKEN", "").strip()
    chat_id = os.environ.get("V2_STAGING_TELEGRAM_CHAT_ID", "").strip()
    if os.environ.get("V2_STAGING_TELEGRAM_ALLOW_SEND") != "YES":
        raise RuntimeError("Set V2_STAGING_TELEGRAM_ALLOW_SEND=YES to authorize staging sends")
...
async def _run() -> None:
    ...
    async with httpx.AsyncClient(timeout=20) as client:
        ...
        chat = (await client.get(f"{base}/getChat", params={"chat_id": staging.telegram_chat_id})).json()
