...
Run Alembic first, then set ``V2_PROOF_DATABASE_URL``.
"""
...
from leetcode_coach_v2 import jobs as jobs_module
...
from leetcode_coach_v2.integrations.leetcode import ProblemRecord
...
async def _run() -> None:
    ...
    database_url = os.environ.get("V2_PROOF_DATABASE_URL", "")
    local_sqlite = database_url.startswith("sqlite:///") and (
        ".local-v2-live-proof.db" in database_url
    )
    local_postgres = (
        os.environ.get("V2_PROOF_ALLOW_POSTGRES") == "1"
        and database_url
        == "postgresql+psycopg://proof:proof@127.0.0.1:55432/proof"
    )
    if not local_sqlite and not local_postgres:
        raise SystemExit(
            "V2_PROOF_DATABASE_URL must target .local-v2-live-proof.db or the explicitly "
  ...
  void dfs(char[][] g, int r, int c) {
    ...
    with Session(engine) as session:
        latest_run = session.exec(
            select(V2AgentRun).order_by(V2AgentRun.started_at.desc())
        ).first()
    ...
    with Session(engine) as session:
        ...
        refill = session.exec(
            select(V2ProposalBatch).order_by(V2ProposalBatch.id.desc())
        ).first()
        assert refill is not None and refill.telegram_message_id is not None
        refill_id = refill.id
    await app.handle_callback(
        chat_id=chat_id, callback_id="pick-one", data=f"v2p:{refill_id}:1"
    )
    await app.handle_callback(
        chat_id=chat_id, callback_id="pick-two", data=f"v2p:{refill_id}:2"
    )
    ...
    from leetcode_coach_v2 import main as main_module
